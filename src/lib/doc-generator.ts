...
function wrapText(text: string, maxLength: number): string {
  ...
  if (lastSpaceIndex === -1) {
    return text.substring(0, maxLength) + '<br>' + wrapText(text.substring(maxLength), maxLength);
  }
  
  return text.substring(0, lastSpaceIndex) + '<br>' + wrapText(text.substring(lastSpaceIndex + 1), maxLength);
}

/**
 * Format workflow inputs, outputs, and secrets in stacked format for compact display
 */
function formatStackedWorkflowDetails(columns: Record<string, unknown> | undefined): string {
...
const CATEGORY_MAPPINGS = [
  ...
  { prefixes: ['deploy-', 'build-', 'publish-', 'release-'], category: 'Deployment', icon: '🚀' },
  { prefixes: ['security-', 'scan-', 'audit-'], category: 'Security', icon: '🔒' },
  { prefixes: ['util-', 'setup-', 'tool-', 'maintenance-'], category: 'Utilities', icon: '🛠️' },
  { prefixes: ['doc-', 'docs-', 'generate-'], category: 'Documentation', icon: '📚' },
  { prefixes: ['notify-', 'alert-'], category: 'Notifications', icon: '🔔' },
  { prefixes: ['monitor-', 'check-', 'health-'], category: 'Monitoring', icon: '�' },
] as const;

/**
 * Get category and icon based on filename
 */
function getCategoryByFilename(filename: string): { category: string; icon: string } {
...
function categorizeWorkflow(workflow: Workflow): {
  ...
  const filename = workflow.filePath.split(/[/\\]/).pop()?.replace('.yml', '').replace('.yaml', '') || '';
  ...
  if (workflowFiles.length > 0) {
    ...
    for (let i = 0; i < sortedWorkflowFiles.length; i++) {
      ...
      if (category !== lastCategory) {
        ...
        const hasMoreFiles = i < sortedWorkflowFiles.length - 1 || actionDirs.length > 0;
        structure += `│   ${hasMoreFiles ? '├──' : '└──'} # ${icon} ${category}\n`;
        lastCategory = category;
      }
      
      const isLastFile = i === sortedWorkflowFiles.length - 1 && actionDirs.length === 0;
      let prefix = '├──';
      if (isLastFile) {
        prefix = '└──';
      }
...
export async function generateDocumentation(workflows: Workflow[]): Promise<void> {
  ...
  const categoriesMap = new Map<string, { icon: string; workflows: Workflow[] }>();
  for (const workflow of reusableWorkflows) {
    ...
    if (existing) {
      ...
      categoriesMap.set(category.category, { icon: category.icon, workflows: [workflow] });
    }
  }
  ...
  if (categoriesMap.size === 0) {
    ...
    for (const [category, { icon, workflows }] of categoriesMap) {
      ...
      workflowsSection += '| Workflow Name | Description | Inputs | Outputs | Secrets |\n';
      workflowsSection += '|:-------------|:------------|:-------|:--------|:--------|\n';
  ...
  const availableWorkflowsStart = existingContent.indexOf('<!-- BEGIN: Available Workflows Section -->');
  const folderStructureEnd = existingContent.indexOf('<!-- END: Workflows Folder Structure Section -->');
  
  const beforeWorkflows = existingContent.substring(0, availableWorkflowsStart);
  const afterFolderStructureEnd = folderStructureEnd + '<!-- END: Workflows Folder Structure Section -->'.length;
  const afterFolderStructure = existingContent.substring(afterFolderStructureEnd);
