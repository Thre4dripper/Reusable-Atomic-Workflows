...
| **[Build Debug APK](./.github/workflows/build-debug-apk.yml)**     | Generate a debug APK for an Android project using<br>customizable inputs.     | `java-version`<br>`java-distribution`<br>`gradle-cache`<br>`checkout-ref`<br>`secret-files-artifact-name`<br>`secret-files-path`<br>`gradle-build-command`<br>`apk-output-path`<br>`artifact-name`<br>`artifact-retention-days`                                                                                      | _None_  | _None_                                                                    |
